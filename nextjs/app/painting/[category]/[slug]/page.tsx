...
export default function PaintingDetailPage({ params }: { params: any }) {
			...
			// See: https://nextjs.org/docs/messages/params-promise
					...
					const { category, slug: initialSlug } = resolvedParams;
	...
	const [lightboxOpen, setLightboxOpen] = useState(false);
		...
		useEffect(() => {
			if (typeof window !== 'undefined') {
				setLightboxOpen(new URLSearchParams(window.location.search).get('lightbox') === '1');
			}
		}, []);
	...
	// Find current, previous, and next paintings in this category
		...
		// Only set prev/next if idx is valid and paintings exist
		...
		return (
			...
			>
						<PaintingDetailClient
							img={img}
							loading={loading}
							prev={prev && prev.link ? { href: `/painting/${category}/${prev.link}`, name: String(prev.name), slug: String(prev.link) } : undefined}
							next={next && next.link ? { href: `/painting/${category}/${next.link}`, name: String(next.name), slug: String(next.link) } : undefined}
							current={current && current.link ? { href: `/painting/${category}/${current.link}`, name: String(current.name), slug: String(current.link) } : undefined}
				...
				{img && (
					<>
						...
						<div style={{
							...
							fontWeight: 500,
							letterSpacing: 0.1,
						}}>{img.author} &middot; {img.category}</div>
						<div style={{ width: '100%' }}>
